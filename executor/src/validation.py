...
class ValidationPipeline:
    ...
    Executa a suíte de testes e linters configurados dentro do sandbox efêmero Docker ativo.
    """
    ...
    def execute_command(
        ...
        """
        Executa um único comando de validação no sandbox e retorna um ValidationResult estruturado.
        """
        start_time = time.time()
        try:
            if session:
                res = session.exec(command=command, env_vars=env_vars, timeout=timeout)
            else:
